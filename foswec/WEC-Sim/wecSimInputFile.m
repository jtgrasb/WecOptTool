...
body(2).mass = 0.2*(.05*.76*.56)*1000; % mass of flaps is half of displaced mass to add buoyancy
body(2).inertia = [0 (1/12)*0.2*(.05*.76*.56)*1000*(0.05^2 + 0.56^2) 0];

% flap 2
body(3) = bodyClass('hydroData/foswec.h5'); 
body(3).geometryFile = 'geometry/flap.stl'; 
body(3).mass = 0.2*(.05*.76*.56)*1000;                   
body(3).inertia = [0 (1/12)*0.2*(0.05*0.76*0.56)*1000*(0.05^2 + 0.56^2) 0];
...
mooring(1).location = [0, 0, -0.46];
mooring(1).matrix.stiffness = diag([3*1e4 0 1e5 0 1e6 0]);
mooring(1).matrix.damping = diag([3*1e3 0 1e4 0 1e5 0]);
mooring(1).matrix.preTension(3) = -9.81*2*(1-0.2)*(.05*.76*.46)*1000; % compensates for buoyancy from flaps
